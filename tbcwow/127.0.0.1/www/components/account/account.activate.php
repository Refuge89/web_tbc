...
// ==================== //
	...
	$get_lock_on_accid = $DB->select("SELECT locked FROM account WHERE id=".$user['id']."");
...
if($key){
  if($user['id'] = $auth->isvalidactkey($key)){
    $DB->query("UPDATE account SET locked=0 WHERE id=?d LIMIT 1",$act_accid);
        $DB->query("UPDATE account_extend SET activation_code=NULL WHERE account_id=?d LIMIT 1",$user['id']);
