...
if($user['g_use_attach']==1){  
    ...
    $all_attachs_count = 0;
    $attachs = array();
    $user_attach_dir = $config['attachs_path'].$user['id'].'/';
    ...
    if($all_attachs_size <= $config['max_attachs_size'])$this['allowupload'] = true;else $this['allowupload'] = false;
            ...
            if($tmpattach['size'] <= $config['max_attachs_size']-$all_attachs_size){
                ...
                if(in_array($ext,$allowed_ext)){
                    if(@move_uploaded_file($tmpattach['tmp_name'], $user_attach_dir.$tmpattach['name'])){
                        ...
                        redirect($config['base_href'].'index.php?n=forum&sub=attach&tid='.$_GET['tid'],1);
                    }
                }
            }
        }
