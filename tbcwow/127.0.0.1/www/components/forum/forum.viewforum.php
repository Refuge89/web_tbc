...
if($this_forum['forum_id']<=0)exit('This forum dose not exists.');
$this_forum['linktonewtopic'] = $config['site_href'].'index.php?n=forum&sub=post&action=newtopic&f='.$this_forum['forum_id'].'';
$this_forum['linktomarkread'] = $config['site_href'].'index.php?n=forum&sub=viewforum&fid='.$this_forum['forum_id'].'&markread=1';
// ==================== //
...
//===== Calc pages =====//
$items_per_pages = $config['topics_per_page'];
...
foreach($alltopics as $cur_topic)
{
    ...
    $pnum = ceil($cur_topic['num_replies']/$config['posts_per_page']);
    if($pnum>1){
    ...
    $cur_topic['linktothis'] = $config['site_href'].'index.php?n=forum&sub=viewtopic&tid='.$cur_topic['topic_id'].'';
    $cur_topic['linktolastpost'] = $config['site_href'].'index.php?n=forum&sub=viewtopic&tid='.$cur_topic['topic_id'].'&to=lastpost';
    $cur_topic['linktoprofile1'] = $config['site_href'].'index.php?n=account&sub=view&action=find&name='.$cur_topic['username'].'';
    $cur_topic['linktoprofile2'] = $config['site_href'].'index.php?n=account&sub=view&action=find&name='.$cur_topic['last_poster'].'';
