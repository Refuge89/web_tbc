...
class DbSimple_Generic_Database extends DbSimple_Generic_LastError
{
    ...
    function _query($query, &$total)
    {
        ...
        // Log resulting query now, because later we may have query error,
        // and it should be placed AFTER logged query.
    ...
    function _transformResult($rows)
    {
        ...
        if (is_array($rows) && $rows) {
            $ak = DBSIMPLE_ARRAY_KEY;
            $pk = DBSIMPLE_PARENT_KEY;
            if (array_key_exists($ak, $rows[0])) {
                // Tree-based array? Fields: ARRAY_KEY, PARENT_KEY
                if (array_key_exists($pk, $rows[0])) {
                    return $this->_transformResultToForest($rows, $ak, $pk);
                }
    ...
    /**
     * Convert rowset to key-based array.
     */
    function _transformResultToHash($rows, $ak)
    {
        $result = array();
        foreach ($rows as $row) {
            $k = $row[$ak];
