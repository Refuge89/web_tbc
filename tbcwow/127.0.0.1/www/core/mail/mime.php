...
class MIME {
	...
	function enc_header($str, $charset = 'utf-8', $encoding = 'quoted-printable'){
		...
		}elseif($encoding == 'base64') $enc = chunk_split(base64_encode($str), $this->_hchunklen, $this->_crlf);
		if($enc){
			$res = array();
			$arr = explode($this->_crlf, $enc);
			$chr = ($encoding == 'base64') ? 'B' : 'Q';
			foreach($arr as $val) if($val != '') $res[] = '=?'.$charset.'?'.$chr.'?'.$val.'?=';
