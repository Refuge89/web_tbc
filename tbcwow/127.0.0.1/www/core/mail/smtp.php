...
class SMTP extends MIME {
	...
	function SMTP(){
		...
		$this->_arrenc = array('7bit' => '', '8bit' => '', 'quoted-printable' => '', 'base64' => '');
	}
	...
	function addheader($hname, $hvalue, $charset = 'utf-8', $encoding = 'quoted-printable'){
		...
		if(is_string($charset)){
			...
			if(!($charlen > 1 && $charlen < 60)){
				$charset = 'utf-8';
				trigger_error('Invalid charset value, on class SMTP::addheader()', 512);
			}
		}else{
			$charset = 'utf-8';
			trigger_error('Invalid charset type value, on class SMTP::addheader()', 512);
		}
		if(is_string($encoding)){
			...
			if(!($encoding != 'quoted-printable' || $encoding != 'base64')){
				...
				trigger_error('Invalid encoding value, on class SMTP::addheader()', 512);
			}
		}else{
			$encoding = 'quoted-printable';
			trigger_error('Invalid encoding type value, on class SMTP::addheader()', 512);
		}
		if(is_string($hname)){
				...
				if(is_string($hvalue)){
					...
					if($hvalue != ""){
						...
						elseif($vname == "mime-version") trigger_error('Can not set "MIME-Version" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
							...
							$ret = true;
							$this->_header[] = array('name' => ucfirst($hname), 'value' => $this->enc_header($hvalue, $charset, $encoding));
						}
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				if(!isset($this->_toaddrs[$adrr])){
					...
					if(is_string($name)){
						...
						$name = trim($name);
						if($name != "") $this->_toaddrs[$adrr] = $name;
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				if(!isset($this->_ccaddrs[$adrr])){
					...
					if(is_string($name)){
						$name = FUNC::str_clear($name);
						$name = trim($name);
						if($name != "") $this->_ccaddrs[$adrr] = $name;
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				...
				if(is_string($name)){
					$name = FUNC::str_clear($name);
					$name = trim($name);
					if($name != "") $this->_fromaddr = array('address' => $adrr, 'name' => $name);
	...
	function text($text, $charset = 'us-ascii', $encoding = 'quoted-printable', $disposition = 'inline'){
	...
	function html($html, $charset = 'us-ascii', $encoding = 'quoted-printable', $disposition = 'inline'){
	...
	function _encodemsg($src, $enc){
		$res = '';
		if($enc == "7bit" || $enc == "8bit") $res .= chunk_split($src, $this->_chunklen, $this->_crlf);
		elseif($enc == "base64") $res .= chunk_split(base64_encode($src), $this->_chunklen, $this->_crlf);
		elseif($enc == "quoted-printable") $res .= $this->qpencode($src, $this->_chunklen, $this->_crlf);
		return $res;
	}
	...
	function _writemsg(){
		...
		$xmail = array('name' => base64_decode('WC1NYWlsZXI='), 'value' => base64_decode('WFBNMiB2LjAuMiA8d3d3LnhwZXJ0bWFpbGVyLmNvbT4='));
		$arrval1[] = $xmail;
		$arrval2[] = $xmail;
		$hval1 = $hval2 = $bval = '';
	...
	function send($subject, $charset = 'utf-8', $encoding = 'quoted-printable'){
		...
		if(is_string($charset)){
			...
			if(!($charlen > 1 && $charlen < 60)){
				$charset = 'utf-8';
				trigger_error('Invalid charset value, on class SMTP::send()', 512);
			}
		}else{
			$charset = 'utf-8';
			trigger_error('Invalid charset type value, on class SMTP::send()', 512);
		}
		if(is_string($encoding)){
			$encoding = FUNC::str_clear($encoding, array(' '));
			$encoding = trim(strtolower($encoding));
			if(!($encoding != 'quoted-printable' || $encoding != 'base64')){
				$encoding = 'quoted-printable';
				trigger_error('Invalid encoding value, on class SMTP::send()', 512);
			}
		}else{
			$encoding = 'quoted-printable';
			trigger_error('Invalid encoding type value, on class SMTP::send()', 512);
		}
		if(is_string($subject)){
			$subject = FUNC::str_clear($subject);
			$subject = trim($subject);
			if($subject != ""){
				$this->_subject = $this->enc_header($subject, $charset, $encoding);
				$ver = true;
