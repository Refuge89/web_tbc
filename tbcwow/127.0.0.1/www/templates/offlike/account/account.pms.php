...
<div>
    <ul class="mybulleted_list">
        ...
        <li <?php if($_GET['action']=='view' && $_GET['dir']=='in')echo'class="selected"';?>><a href="index.php?n=account&sub=pms"><?php echo $lang['inbox'] ?></a></li>
        <li <?php if($_GET['action']=='view' && $_GET['dir']=='out')echo'class="selected"';?>><a href="#" onClick="return false;"><s><?php echo $lang['outbox']?></s></a></li>
    </ul>
<?php if($_GET['action']=='view' && $_GET['dir']=='in'){ ?>
...
function Check(type)
{
  ...
  for (var i=0;i<fmobj.elements.length;i++)
  {
    ...
    <form method="post" action="index.php?n=account&sub=pms" name="mutliact">
...
<?php foreach($items as $item){ ?>
                <tr class="normal <?php echo ($item['showed']=='1'?'read':'unread')?>">
                    ...
                    <td class="n_title"><a href="index.php?n=account&sub=pms&action=viewpm&iid=<?php echo $item['id']; ?>"><?php echo $item['subject']; ?></a></td>
                    <td class="n_time" width="140"><a href="index.php?n=account&sub=pms&action=viewpm&iid=<?php echo $item['id']; ?>"><?php echo date('d-m-Y, H:i',$item['posted']);?></a></td>
                </tr>
<?php } ?>
            </tbody>
        </table>
        ...
        <tbody>
            <tr>
                <td width="120" rowspan="2" valign="top" align="center">
                    <img src="<?php echo $config['avatar_path'];?><?php echo $senderinfo['avatar'];?>" border="0"/><br>
