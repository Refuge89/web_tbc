...
<ul style="font-weight:bold;list-style:none;">
    <?php foreach($allkeys as $key){
        if($key['used']==0)echo'<li><a href="index.php?n=admin&sub=keys&action=delete&keyid='.$key['id'].'" title="Delete"><img src="'.$config['template_href'].'images/icons2/delete.png" alt="[delete]" align="absmiddle"></a>&nbsp; <a href="index.php?n=admin&sub=keys&action=setused&keyid='.$key['id'].'" title="Mark as used">'.$key['id'].') '.$key['key'].'</a></li>'."\n";
        else echo'<li><a href="index.php?n=admin&sub=keys&action=delete&keyid='.$key['id'].'" title="Delete"><img src="'.$config['template_href'].'images/icons2/delete.png" alt="[delete]" align="absmiddle"></a>&nbsp; <s>'.$key['id'].') '.$key['key'].'</s></li>'."\n";
    } ?>
</ul>