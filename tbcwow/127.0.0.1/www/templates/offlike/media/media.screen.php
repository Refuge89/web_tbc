<?php
    ...
    if ($_POST['doadd'] != ''){
     ...
     echo $lang['Filetype'];
     echo ("<br>"); }
     if($DB->selectCell("SELECT img FROM `gallery` WHERE img='$img' AND cat='screenshot'") == $img){
     echo $lang['ErrorFilename'];
     exit;
     }
     if(copy($_FILES["filename"]["tmp_name"],
     "./images/screenshots/".$_FILES["filename"]["name"])) {
     $DB->query("INSERT INTO gallery (img,comment,autor,date,cat) VALUES('$img','$comment','$autor','$date','screenshot')");
     } else {
     echo $lang['Uploaderror']; }
    }
?>
<?php
$gal = $DB->selectCell("SELECT count(*) FROM `gallery` WHERE cat='screenshot'");
?>


<table border = 0 width=100%>
<?php if($user['id']>=1){ ?>
<tr><td ><img src="<?php echo $config['template_href']; ?>images/edit-button.gif"><a href="././index.php?n=media&sub=addgalscreen"><?echo $lang['Addimage'];?></a></td>
...
<tr>
<?php
$sql = $DB->select("SELECT * FROM gallery WHERE cat='screenshot'");
...
<td>
<a style="cursor: pointer;" onclick="javascript:void(window.open('./images/screenshots/<?PHP echo  $tablerows['img'];?>'))" target="_blank"><img style="width: 235px; height: 175px;" alt="<?php echo  $tablerows['comment'];?>"src="./images/screenshots/<?php echo  $tablerows['img'];?>" border="0"></a>
</td>
